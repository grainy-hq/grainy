  ...
  return (
    <GlassCard className="p-5">
      ...
      >
        ...
        {unseenCount > 0 && (
          <span className="bg-accent flex size-5 items-center justify-center rounded-full text-[10px] font-bold text-white">
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-1" onClick={markSeen}>
          ...
          {notifications.length > 5 && (
            <Link
              href="/notifications"
              className="text-accent hover:text-accent-hover block pt-1 text-center text-[11px] transition"
            >
              View all
            </Link>
          )}
      ...
      <div className="relative shrink-0">
        {item.actor?.image ? (
          <img
            ...
            alt=""
            className="mt-0.5 size-8 rounded-full object-cover ring-1 ring-white/10"
          />
        ) : (
          <div className="bg-accent/20 text-accent mt-0.5 flex size-8 items-center justify-center rounded-full text-xs font-semibold">
            {item.actor?.name?.[0] ?? "?"}
          </div>
        )}

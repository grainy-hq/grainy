...
const PRESETS: { name: string; theme: Partial<CustomTheme> }[] = [
  {
name: "Ocean",
    theme: { bg: "#0a1628", fg: "#e0f0ff", accent: "#4a9eff", accentHover: "#6ab4ff", accentSecondary: "#2a7acc", accentTertiary: "#1a5a99", muted: "#8a9ab8" },
  },
  {
name: "Forest",
    theme: { bg: "#0a1a10", fg: "#e0f0e4", accent: "#4a9e6f", accentHover: "#6ab88a", accentSecondary: "#2a7a55", accentTertiary: "#1a5a3d", muted: "#8a9a90" },
  },
  {
name: "Sunset",
    theme: { bg: "#1a0a12", fg: "#f0e0e4", accent: "#e86a4a", accentHover: "#f08a6a", accentSecondary: "#c0553a", accentTertiary: "#903d2a", muted: "#a08a8a" },
  },
  {
name: "Midnight",
    theme: { bg: "#050508", fg: "#d8d8e8", accent: "#a080ff", accentHover: "#b8a0ff", accentSecondary: "#7860cc", accentTertiary: "#5040a0", muted: "#787090" },
  },
  {
name: "Candy",
    theme: { bg: "#1a0a1a", fg: "#f0e0f0", accent: "#ff6aaa", accentHover: "#ff8ac0", accentSecondary: "#cc5088", accentTertiary: "#a03866", muted: "#a08098" },
  },
]
...
if (!profile) {
return <p className="mx-auto max-w-xl py-10 text-center text-muted">Loading...</p>
  }
...
return (
      <div className="mx-auto max-w-xl space-y-8 py-10">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Get a verified blue badge on your profile, posts, and comments.
                  <br />
                  <span className="text-xs text-muted/70">
                    Only if you are a recognized public figure or work directly with Grainy.
                  </span>
                </p>
              </div>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Everything in Verification, plus exclusive green theme, custom profile
                  background, and full Grainy+ badge. Includes verified badge.
                </p>
              </div>
              <div className="shrink-0 text-right">
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          <h3 className="font-semibold mb-3">Activate your purchase</h3>
          <div className="flex gap-2">
            <input
              value={activationCode}
onChange={(e) => setActivationCode(e.target.value)}
placeholder="XXXX-XXXXX"
              className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none uppercase tracking-widest focus:border-accent"
            />
            <button
              onClick={handleActivate}
disabled={activating || !activationCode.trim()}
className="shrink-0 rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-40"
            >
              {activating ? "..." : "Activate"}
            </button>
          </div>
          {activationMsg && (
            <p className="mt-2 text-sm text-accent">{activationMsg}</p>
          )}
        </div>
...
return (
    <div className="mx-auto max-w-2xl space-y-8 py-6">
      <div className="flex items-center gap-3">
        <div className="flex size-10 items-center justify-center rounded-lg bg-accent/20 text-lg">✦</div>
        <div>
          <h1 className="text-2xl font-bold">Grainy Plus</h1>
          <p className="text-sm text-muted">You&apos;re a premium member</p>
        </div>
      </div>

      {msg && (
        <p className="rounded-lg bg-accent/10 px-3 py-2 text-sm text-accent">{msg}</p>
      )}

      <div className="rounded-lg border border-border bg-card p-6">
        <h2 className="font-semibold mb-4">Preset themes</h2>
...
onChange={(e) => handleChange(key as keyof CustomTheme, e.target.value)}
className="size-9 cursor-pointer rounded-lg border border-border bg-transparent p-0.5"
                ...
                <input
                  ...
                  value={ct[key as keyof typeof ct] as string}
onChange={(e) => handleChange(key as keyof CustomTheme, e.target.value)}
className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-xs font-mono outline-none focus:border-accent"
                />
              </div>
            </div>
          ))}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted">Background style</label>
            <select
              value={ct.bgStyle}
...
className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent"
          ...
          {ct.bgStyle === "gradient" && (
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Gradient CSS</label>
              <input
                type="text"
                value={ct.bgGradient ?? ""}
...
placeholder="linear-gradient(135deg, #0a0a12, #1a1a2e)"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Card opacity</label>
              <input
                type="number"
                min={0}
max={1}
...
onChange={(e) => handleChange("cardOpacity", parseFloat(e.target.value))}
className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs font-mono outline-none focus:border-accent"
              />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Glass blur</label>
              <input
                type="text"
                value={ct.glassBlur ?? ""}
...
placeholder="20px"
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div
          ...
          style={{
background: ct.bgStyle === "gradient" && ct.bgGradient ? ct.bgGradient : ct.bg,
            color: ct.fg,
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: ct.muted }}>@janedoe</p>
            </div>
          </div>
          <p className="text-sm leading-relaxed" style={{ color: ct.fg + "dd" }}>
      ...
      <button
        onClick={handleSave}
disabled={saving}
className="w-full rounded-lg bg-accent py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-40"
      >

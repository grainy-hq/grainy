...
export default function SettingsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const body: Record<string, unknown> = {
      ...
      bio,
      location,
      profileBackground: bannerUrl || null,
    }
    if (selectedTrack) {
  ...
  return (
    <div className="mx-auto max-w-xl space-y-8">
      ...
      >
          ...
          <div className="flex items-center gap-4">
            {profile.image ? (
              <img
                src={profile.image}
                alt=""
                className="ring-accent/30 size-16 rounded-full object-cover ring-2"
              />
            ) : (
              <div className="bg-accent/20 text-accent flex size-16 items-center justify-center rounded-full text-lg font-bold">
                {profile.name[0]}
              </div>
            )}
            <label className="border-border hover:bg-border/50 cursor-pointer rounded-full border px-4 py-2 text-sm transition">
              Change photo
              <input
                type="file"
                accept="image/*"
        ...
        <div>
          ...
          {bannerUrl && (
            <div className="relative mb-3 overflow-hidden rounded-xl bg-white/5">
              ...
              <button
                ...
                className="absolute top-2 right-2 rounded-full bg-black/60 px-2 py-0.5 text-xs text-white backdrop-blur transition hover:bg-black/80"
              >
                Remove
              </button>
            </div>
          ...
          <input
            ...
            className="border-border bg-background focus:border-accent w-full rounded-xl border px-4 py-2.5 text-sm outline-none"
          />
          <p className="text-muted mt-1 text-xs">
            Paste an image URL to set as your profile banner.
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            maxLength={500}
            className="border-border bg-background focus:border-accent w-full resize-none rounded-xl border px-4 py-2.5 text-sm outline-none"
          />
        </div>
        ...
        <button
          type="submit"
          disabled={saving}
          className="accent-btn w-full rounded-full py-2.5 text-sm font-semibold text-white disabled:opacity-50"
        >
      ...
      <button
        ...
        className="border-border w-full rounded-full border py-2.5 text-sm font-medium text-red-400 transition hover:bg-red-500/10"
      >
        Sign out
      </button>
    </div>
